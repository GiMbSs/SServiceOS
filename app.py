...
app.config['WTF_CSRF_SECRET_KEY'] = os.getenv('CSRF_SECRET_KEY') or os.urandom(24)
    ...
    if request.method == 'POST':
        ...
        with sqlite3.connect('database.db') as conn:
            ...
            if usuario:
                ...
                # Verificar se a senha está em texto puro ou hash bcrypt
                if len(usuario[3]) == 60 and usuario[3].startswith('$2b$'):
                    # Senha em formato bcrypt
                    if not bcrypt.checkpw(senha, usuario[3].encode('utf-8')):
                        flash('Senha incorreta!', 'danger')
                        return redirect(url_for('login'))
                else:
                    # Senha em texto puro - migrar para bcrypt
                    if usuario[3] != senha.decode('utf-8'):
                        flash('Senha incorreta!', 'danger')
                        return redirect(url_for('login'))
                    
                    # Atualizar para bcrypt
                    hashed_pw = bcrypt.hashpw(senha, bcrypt.gensalt()).decode('utf-8')
                    cursor.execute('UPDATE usuarios SET senha = ? WHERE id = ?', (hashed_pw, usuario[0]))
                    conn.commit()
...
def equipamento():
    if request.method == 'POST':
        ...
        with sqlite3.connect('database.db') as conn:
            ...
            # Gerar código de barras para o equipamento usando abordagem alternativa
            equip_cod = f"EQ{equipamento_id:06d}"  # Formata como "EQ000001"
            
            # Criar código de barras em SVG (não requer Pillow)
            from barcode import Code128
            from barcode.writer import SVGWriter
            
            code = Code128(equip_cod, writer=SVGWriter())
            barcode_path = os.path.join(app.config['UPLOAD_FOLDER'], f'barcode_{equip_cod}')
            code.save(barcode_path)
            
            # Usar SVG diretamente sem tentar conversão para PNG
            barcode_path += '.svg'
...
def visualizar_equipamento(equipamento_id):
    ...
    # Gerar código de barras em SVG para exibição
    equip_cod = equipamento[7]  # código de barras armazenado no banco
    from barcode import Code128
    from barcode.writer import SVGWriter
    barcode_objeto = Code128(equip_cod, writer=SVGWriter())
...
def visualizar_ordem(ordem_id):
    ...
    # Gerar código de barras em SVG para a ordem
    ordem_cod = f"OS{ordem[0]:06d}"  # Formata como "OS000001"
    from barcode import Code128
    from barcode.writer import SVGWriter
    
    barcode_objeto = Code128(ordem_cod, writer=SVGWriter())
...
@login_required
@limiter.limit("10 per hour")  # Limite de 10 cadastros por hora
def usuario():
    if request.method == 'POST':
        nome = request.form['nome']
        email = request.form['email']
